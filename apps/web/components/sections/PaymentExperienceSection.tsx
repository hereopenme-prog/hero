...
export function PaymentExperienceSection() {
  return (
    <Section id="payment-experience" className="bg-[var(--bg)]">
      ...
      <Container className="relative z-10">
        ...
        >
          ...
          <div className="mt-6 flex items-center gap-2">
            ...
            <span className="font-body text-[0.8rem] text-[var(--ink-2)]">Payment received.</span>
          </div>
        </motion.div>
            ...
            {stripItems.map((item, i) => (
              <div key={item.label} className="flex items-center gap-4 lg:gap-8">
                <div className="flex flex-col items-start">
                  <span className="font-display font-bold text-base lg:text-xl tracking-tight text-[var(--ink)]">
                    {item.label}
                  </span>
                  <span className="mt-1 font-body text-[0.72rem] text-[var(--ink-muted)]">{item.caption}</span>
                </div>
                {i < stripItems.length - 1 && (
                  <div className="hidden sm:flex flex-col items-center justify-center self-center">
                    <span className="w-px h-4 bg-[var(--accent)] block" />
                    <svg width="10" height="6" viewBox="0 0 10 6" fill="none" className="text-[var(--accent)]">
