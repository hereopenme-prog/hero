...
const pillars = [
  {
    icon: <Activity size={22} className="text-[#00D084]" strokeWidth={1.5} />,
  ...
  {
    icon: <Radar size={22} className="text-[#00D084]" strokeWidth={1.5} />,
    title: 'Multi-Sensor Integration',
    caption: 'Designed to support multiple sensing capabilities over time.',
  },
  {
    icon: <Cpu size={22} className="text-[#00D084]" strokeWidth={1.5} />,
  ...
  {
    icon: <MousePointerClick size={22} className="text-[#00D084]" strokeWidth={1.5} />,
...
export function InnovationSection() {
  return (
    <Section id="innovation" className="bg-[#080C10]">
      <Container className="relative z-10">
          ...
          {pillars.map((p, i) => {
            ...
            return (
              <motion.div
                ...
                variants={stagger}
                className={`flex flex-col sm:flex-row items-start gap-5 sm:gap-8 border-b border-[#1C2A38] pb-10 ${
                  reversed ? 'sm:flex-row-reverse text-left' : 'text-left'
                }`}
              >
                <motion.div
                  variants={fadeUp}
                  className="flex-shrink-0 flex items-center justify-center rounded-2xl border border-[#00D08420]"
                  style={{ background: '#00D0840A', width: 56, height: 56 }}
                >
                  {p.icon}
                </motion.div>
                ...
                <div className={`sm:${reversed ? 'text-right' : 'text-left'} flex-1`}>
                  <motion.h3
                    variants={fadeUp}
                    className="font-display font-bold text-xl sm:text-2xl tracking-tight text-[#E8EDF2]"
                  >
                    {p.title}
                  </motion.h3>
                  <motion.p
                    variants={fadeUp}
                    className="mt-2 font-body text-[0.95rem] text-[#8A9BAE] leading-relaxed"
                  >
                    {p.caption}
                  </motion.p>
                </div>
        ...
        <motion.p
          ...
          className="mt-16 text-center font-display font-bold text-[1.4rem] lg:text-[1.8rem] tracking-tight text-[#E8EDF2]"
        >
          One device, designed to become a{' '}
          <span className="text-[#00D084]">connected business infrastructure.</span>
        </motion.p>
      </Container>
    </Section>
  );
