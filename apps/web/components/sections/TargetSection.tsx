...
export function TargetSection() {
  return (
    <Section id="target" className="bg-[#080C10]">
        ...
        >
          {businesses.map((b) => (
            <motion.div
              key={b.label}
              variants={scaleIn}
              className="group flex items-center gap-3 rounded-xl border border-[#1C2A38] bg-[#0F1923] px-4 py-4 transition-colors duration-200 hover:border-[#00D08440]"
            >
              <div
                className="flex-shrink-0 flex items-center justify-center transition-transform duration-300 group-hover:-translate-y-0.5"
                style={{ background: '#00D0841A', borderRadius: 8, padding: 8, width: 36, height: 36 }}
              >
                {b.icon}
              </div>
              <p className="font-display font-semibold text-[0.8rem] tracking-wide leading-snug text-[#E8EDF2]">
                {b.label}
              </p>
            </motion.div>
          ))}
