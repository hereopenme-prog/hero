...
export function MoreThanStatusSection() {
  return (
    <Section id="more-than-status" className="bg-[var(--bg)]">
      ...
      <Container className="relative z-10">
        ...
        >
          ...
          <div className="hidden lg:flex flex-col gap-10">
            <div className="grid grid-cols-8 gap-0 items-start">
              {evolution.map((node, i) => (
                ...
                >
                  <div className="relative flex items-center justify-center h-14 w-14 rounded-full border border-[var(--a30)] bg-[var(--section)]">
                    <node.icon size={20} className="text-[var(--accent)]" strokeWidth={1.5} />
                  </div>
                  <div className="mt-4 flex flex-col items-center gap-1">
                    <div className="flex items-center gap-1">
                      <div className="h-1.5 w-1.5 rounded-full bg-[var(--accent)]" aria-hidden="true" />
                      <div className="h-px w-4 bg-[var(--a40)]" aria-hidden="true" />
                    </div>
                    <h3 className="font-display font-bold text-[0.72rem] tracking-[0.08em] text-[var(--ink)] leading-tight text-center">
                      {node.label}
                    </h3>
                    <p className="font-body text-[0.65rem] text-[var(--ink-muted)] leading-snug text-center max-w-[100px]">
                      {node.caption}
                    </p>
                  </div>
                </motion.div>
            ...
            <div className="relative -mt-2" aria-hidden="true">
              <div className="absolute inset-x-[4%] top-1/2 h-px bg-[var(--a25)]" />
              <div className="relative flex justify-between">
                {evolution.map((n) => (
                  <span key={n.label} className="inline-block h-2 w-2 rounded-full bg-[var(--accent)] relative -top-[7px] -translate-y-1/2" />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="lg:hidden flex flex-col items-center max-w-xs mx-auto gap-0">
            {evolution.map((node, i) => (
              ...
              >
                <div className="flex items-center justify-center h-14 w-14 rounded-full border border-[var(--a30)] bg-[var(--section)]">
                  <node.icon size={20} className="text-[var(--accent)]" strokeWidth={1.5} />
                </div>
                <h3 className="mt-4 font-display font-bold text-[0.85rem] tracking-[0.06em] text-[var(--ink)] text-center">
                  {node.label}
                </h3>
                <p className="mt-1 max-w-[200px] font-body text-[0.8rem] text-[var(--ink-muted)] leading-snug text-center">
                  {node.caption}
                </p>
                {i < evolution.length - 1 && (
                  <div className="my-5 flex flex-col items-center text-[var(--a60)]" aria-hidden="true">
                    <div className="h-5 w-px bg-[var(--a30)]" />
        ...
        <motion.p
          ...
          className="mx-auto mt-20 max-w-[680px] text-center font-display font-semibold text-[1.05rem] lg:text-[1.15rem] text-[var(--ink)] tracking-tight"
        >
